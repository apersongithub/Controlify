...
public class AbstractContainerScreenProcessor<T extends AbstractContainerScreen<?>> extends ScreenProcessor<T> {
    ...
    protected void handleScreenVMouse(ControllerEntity controller, VirtualMouseHandler vmouse) {
        ...
        var ctx = new ContainerCtx(hoveredSlot.get(), screen.getMenu().getCarried(), accessor.controlify$invokeHasClickedOutside(vmouse.getCurrentX(1f), vmouse.getCurrentY(1f), accessor.controlify$getLeftPos(), accessor.controlify$getTopPos() /*? if <1.21.9 >>*//*,0*/ ), controller, controller.genericConfig().config().guideVerbosity);
